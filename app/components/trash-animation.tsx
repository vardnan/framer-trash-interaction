...
export function TrashAnimation() {
  ...
  const imagesToShow = readyToRemove ? IMAGES.filter((img) => !imagesToRemove.includes(img)) : IMAGES;
  ...
  return (
    <div className="relative flex h-[500px] flex-col items-center justify-center">
            ...
            return (
              <li key={image} className="relative flex h-[100px] w-[100px]">
                ...
                <button
                  ...
                  onClick={() => {
                    if (isSelected) {
                      setImagesToRemove((images) => images.filter((img) => img !== image));
                    } else {
      ...
      {imagesToRemove.length > 0 && !readyToRemove ? (
        <div className="absolute bottom-8 flex gap-1 rounded-xl p-1 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0px_8px_8px_-8px_rgba(0,0,0,0.16)] will-change-transform">
